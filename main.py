...
trainer.train([
    ...
    "Amazon SageMaker is designed for high availability. There are no maintenance windows or scheduled downtimes. SageMaker APIs run in Amazons proven, high-availability data centers, with service stack replication configured across three facilities in each AWS region to provide fault tolerance in the event of a server failure or Availability Zone outage.",
    ...
    "Yes. You can stop a job at any time. When an Amazon SageMaker Autopilot job is stopped, all ongoing trials will be stopped and no new trial will be started. Build Models",
    ...
    "You can run automatic model tuning in Amazon SageMaker on top of any algorithm as long as its scientifically feasible, including built-in SageMaker algorithms, deep neural networks, or arbitrary algorithms you bring to SageMaker in the form of Docker images.",
    ...
    "Yes. You can even select a heterogeneous cluster where the training can run on a GPU instance and the simulations can run on multiple CPU instances. Deploy Models",
    ...
    "To see a list of support regions, view the AWS region table. Learn more about Amazon SageMaker pricing V",
    "Have more questions?",
    "Contact us Page Content General Build Models Train Models Deploy Models Sign In to the Console Learn About AWS",
])
